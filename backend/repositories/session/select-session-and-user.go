...
func (r *Repository) SelectSessionAndUser(token string) (types.Session, types.User, error) {
	...
	query := `
		SELECT
			s.id, s.user_id, s.token, s.expires_at, s.created_at, s.ip_address, s.user_agent, s.last_seen,
	...
	err := r.db.QueryRow(query, token).Scan(
		&session.ID, &session.UserID, &session.Token, &session.ExpiresAt, &session.CreatedAt, &session.IPAddress, &session.UserAgent, &session.LastSeen,
