...
func main() {
	...
	if migrationType == "force" {
		cmd = exec.Command("migrate", "-database", dbURL, "-path", "db/migrations", "force", strconv.Itoa(targetVersion))
	} else {
		cmd = exec.Command("migrate", "-database", dbURL, "-path", "db/migrations", "goto", strconv.Itoa(targetVersion))
	}
