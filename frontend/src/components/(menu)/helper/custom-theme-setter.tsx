...
export const CustomThemeSetter = ({ children, target, menu }: Props) => {
  ...
  const lightPalette = useMemo(
    () => generateSimplifiedTailwindPalette(colors.light || "#ffffff"),
    [colors.light],
  );

  const darkPalette = useMemo(
    () => generateSimplifiedTailwindPalette(colors.dark || "#000000"),
    [colors.dark],
  );
...
export const MemoizedCustomThemeSetter = memo(CustomThemeSetter);
