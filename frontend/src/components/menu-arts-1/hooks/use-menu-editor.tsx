...
interface DataState {
  ...
  category: {
    ...
    item: {
      ...
      addCosmetic: (categoryId: number) => void;
...
// statusSlice.ts
const createStatusSlice = (set: Set) => ({
  status: {
  ...
  item: {
    ...
    addProduct: (categoryId: number) => {
      set((state) => {
        ...
        const newItem = createNewTranslatableItem<CategoryProduct>(
          locales.map((locale) => locale),
          {
            design: {
              selected: 0,
    ...
    addCosmetic: (categoryId: number) => {
      set((state) => {
        ...
        const newItem = createNewTranslatableItem<CategoryCosmetics>(
          locales.map((locale) => locale),
          {
            design: {
              selected: 0,
